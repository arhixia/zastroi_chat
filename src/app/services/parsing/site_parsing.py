...
"поддержка обычных и JavaScript-рендеримых сайтов"). Playwright, а не
обычный HTTP-запрос (httpx), потому что часть сайтов рисует контент через
JS (SPA на React/Vue и т.п.) — простой GET вернёт пустой каркас без
...
class PlaywrightFetcher:
    ...
    async def fetch(self, url: str) -> str:
        ...
        try:
            # networkidle — ждём, пока сеть "успокоится" (не было запросов 500мс).
            # Для сайтов с бесконечным поллингом/чатами это может быть избыточно
            # строгим условием — если на реальных сайтах будут таймауты именно
            # по этой причине, стоит попробовать wait_until="load" как более мягкий вариант.
